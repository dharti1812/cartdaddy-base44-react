...
export default function AnalyticsPage() {
  ...
  const getDailyOrders = () => {
    ...
    return Object.entries(dailyData).map(([date, orders]) => ({ date, orders })).slice(-7);
  };

  const getTopRetailers = () => {
    return retailers
      .sort((a, b) => (b.successful_deliveries || 0) - (a.successful_deliveries || 0))
      .slice(0, 5)
      .map(r => ({
        name: r.full_name,
  ...
  const stats = {
    ...
    deliveryRate: orders.length > 0 ? (orders.filter(o => o.status === 'delivered').length / orders.length) * 100 : 0,
    avgDeliveryTime: (() => {
      const delivered = orders.filter(o => o.status === 'delivered' && o.created_date && o.actual_delivery_time);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-white opacity-90 mt-1">Performance insights and business metrics</p>
          </div>
          <Button variant="outline" className="bg-white text-[#075E66] border-2 border-[#F4B321] hover:bg-[#F4B321] hover:text-gray-900 font-bold">
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.avgDeliveryTime.toFixed(0)}m</p>
                </div>
              ...
              {loading ? (
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={getDailyOrders()}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="orders" stroke="#1e3a8a" strokeWidth={2} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
