...
export const AuthApi = {
  ...
  sendOTPtoMobile: async (phone, user_type) => {
    localStorage.removeItem("access_token");
    const res = await fetch(`${API_BASE_URL}/api/send-otp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ phone, user_type }),
    });
  ...
  sendOTPtoEmail: async (email, phone, user_type, address, name) => {
    console.log("🔍 Sending email OTP to:", email, "for phone:", phone);
    try {
      const res = await fetch(`${API_BASE_URL}/api/send-email-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, phone, user_type, address, name }),
      });
  ...
  verifyOTPtoEmail: async (email, phone, email_otp, user_type, address, name) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/verify-email-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, phone, email_otp, user_type, address, name }),
      });
