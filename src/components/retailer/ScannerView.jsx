...
import { Camera, CameraOff, SwitchCamera, Zap, ZapOff, ZoomIn, ZoomOut, Sun, Moon } from "lucide-react";
...
export default function ScannerView({ onScan, isActive }) {
  ...
  const autoAdjustExposure = async (brightness) => {
    ...
    if (Math.abs(diff) > 20) {
      ...
      newExposure = Math.max(exposureRangeRef.current.min, Math.min(exposureRangeRef.current.max, exposure + step));
    }
  ...
  const preprocessFrame = (canvas, ctx, video) => {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      if (pixelCount >= target99) { p99 = i; break; }
    }
  ...
  const handleExposureChange = async (delta) => {
    ...
    const track = streamRef.current.getVideoTracks()[0];
    const newExposure = Math.max(exposureRangeRef.current.min, Math.min(exposureRangeRef.current.max, exposure + delta));
    setExposure(newExposure);
    await track.applyConstraints({ advanced: [{ exposureCompensation: newExposure }] });
  };
  ...
  if (!hasCamera) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400">
        ...
        <p className="text-sm text-center max-w-xs">Please allow camera access or try a device with a camera</p>
  ...
  if (!detectorRef.current && !("BarcodeDetector" in window)) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400 p-6">
        <CameraOff className="w-16 h-16" />
        <p className="text-lg font-medium text-center">Barcode Detection Not Supported</p>
        <p className="text-sm text-center max-w-xs">Your browser doesn't support the Barcode Detection API. Please use Chrome on Android or a supported browser.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full h-full bg-black flex flex-col items-center justify-center overflow-hidden">
      <div className="relative w-full max-w-sm h-96 bg-black rounded-lg overflow-hidden border-2 border-cyan-400/30">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="relative w-72 h-20">
            ...
            <div className="absolute left-1 right-1 h-0.5 bg-gradient-to-r from-transparent via-cyan-400 to-transparent animate-pulse"
      ...
      <div className="flex justify-center gap-2 z-10 mt-8 flex-wrap">
        <Button onClick={handleZoomOut} disabled={zoom <= 1} size="icon" className="rounded-full w-12 h-12 bg-white/15 backdrop-blur-md border border-white/20 hover:bg-white/25 text-white disabled:opacity-50">
          <ZoomOut className="w-5 h-5" />
        </Button>
        <Button onClick={toggleFacing} size="icon" className="rounded-full w-12 h-12 bg-white/15 backdrop-blur-md border border-white/20 hover:bg-white/25 text-white">
          <SwitchCamera className="w-5 h-5" />
        </Button>
        <Button onClick={toggleTorch} disabled={!torchSupported} size="icon"
        ...
        <Button onClick={handleZoomIn} size="icon" className="rounded-full w-12 h-12 bg-white/15 backdrop-blur-md border border-white/20 hover:bg-white/25 text-white">
          <ZoomIn className="w-5 h-5" />
        </Button>
        {exposureSupported && (
          <>
            <Button onClick={toggleAutoExposure} size="icon"
            ...
            <Button onClick={() => handleExposureChange(-0.5)} disabled={autoExposure} size="icon"
      ...
      <div className="flex gap-2 mt-4">
        {zoom > 1 && <div className="px-3 py-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white text-xs font-medium">{zoom.toFixed(1)}x</div>}
        {exposure !== 0 && <div className="px-3 py-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white text-xs font-medium">EV {exposure.toFixed(1)}</div>}
      </div>
