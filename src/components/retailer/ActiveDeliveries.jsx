...
import {
  ...
  CheckCircle, Clock, CreditCard, AlertCircle, XCircle, Link as LinkIcon,
  RefreshCw, User as UserIcon // Keeping User as UserIcon, using it for customer details
} from "lucide-react";
...
export default function ActiveDeliveries({ orders, retailerId, config, onUpdate, retailerProfile, onAssignDeliveryBoy, onHandoffDeliveryBoy }) {
  ...
  useEffect(() => {
    ...
    orders.forEach(order => {
      if (order.active_retailer_id === retailerId &&
          ...
          !order.paylink_url) {
        const myAcceptance = order.accepted_retailers?.find(ar => ar.retailer_id === retailerId);
        if (myAcceptance) {
          const acceptedTime = new Date(myAcceptance.accepted_at).getTime();
          const now = Date.now();
          const elapsed = (now - acceptedTime) / 1000;
          const timeout = order.paylink_timeout_sec || config?.paylink_timeout_sec || 150;
  ...
  const handleCancelOrder = async (order) => {
    if (!window.confirm(`Are you sure? You'll be charged ${config?.cancellation_penalty_percentage || 2}% (₹${(order.total_amount * (config?.cancellation_penalty_percentage || 2) / 100).toFixed(0)}) penalty.`)) {
      return;
    }
    ...
    const penaltyAmount = (order.total_amount * (config?.cancellation_penalty_percentage || 2)) / 100;
  ...
  return (
    <>
      {/* Location Tracker for delivery boys */}
      ...
      <div className="space-y-3">
        {orders.length === 0 ? (
          ...
          orders.map((order) => {
            ...
            const isMyDelivery = order.assigned_delivery_boy?.id === currentDeliveryBoy?.id;
            ...
            return (
              ...
              >
                <CardContent className="p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex items-center gap-2 text-gray-700">
                          ...
                          <span className="text-gray-600">{order.customer_masked_contact}</span>
                        </div>
                        ...
                        <div className="mt-3 p-3 bg-white border-2 border-amber-300 rounded-lg">
                          <p className="text-xs font-bold text-amber-900 mb-2">💰 Settlement Information</p>
                          <div className="space-y-1 text-xs">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-bold text-red-700">- ₹{order.delivery_charge || 0}</span>
                            </div>
                            <div className="flex justify-between pt-2 border-t-2 border-amber-200">
                              <span className="font-bold text-gray-900">Your Net Settlement:</span>
                              <span className="font-bold text-green-700">₹{order.seller_net_payable || (order.total_amount - (order.delivery_charge || 0))}</span>
                            </div>
                            <p className="text-[10px] text-gray-600 mt-2 italic">
                              ℹ️ Delivery charges deducted from your settlement & paid to delivery boy
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-3">
                      <Alert className="bg-blue-50 border-blue-500">
                        ...
                        <AlertDescription className="text-blue-900 font-semibold">
                          📱 YOU ARE DELIVERING THIS ORDER - Location tracking is active
                        </AlertDescription>
                      </Alert>
                    </div>
                  )}
                  ...
                  {!isPrimary && (
                    <Alert className="bg-amber-50 border-amber-200 mt-3">
                      ...
                      <AlertDescription className="text-sm text-amber-900">
                        <strong>You're #{position} in line.</strong> Another seller is currently handling this order. You'll be activated if they fail.
                      </AlertDescription>
                    </Alert>
                  )}

                  {isPrimary && order.payment_status === 'needs_paylink' && !order.paylink_url && paylinkTimer && (
                    <Alert className="bg-red-50 border-red-200 mt-3">
                      <AlertCircle className="w-4 h-4 text-red-600" />
                      <AlertDescription className="text-red-900 text-sm">
                        <strong className="text-lg">⏰ {paylinkTimer}s remaining</strong> to generate payment link or order will be reassigned!
                      </AlertDescription>
                    </Alert>
                  )}
                      ...
                      <div className="flex-1">
                        ...
                        {order.distance_km && (
                          <div className="flex items-center gap-2 text-xs text-blue-700">
                            ...
                            <span className="font-medium">{order.distance_km} km from your location</span>
                          </div>
                        )}
                      </div>
                      {isPrimary && (
                  ...
                  {isPrimary && order.payment_status === 'needs_paylink' && !order.paylink_url && (
                    <Button
                  ...
                  {isPrimary && ['paid', 'paylink_sent'].includes(order.payment_status) && (
                    <div className="grid grid-cols-2 gap-3 pt-2 mt-3">
                      ...
                      <Button
                        onClick={() => handleStatusUpdate(order.id, 'en_route')}
                  ...
                  {myAcceptance?.status === 'active' && (
                    ...
                    >
                      ...
                      Cancel Order ({config?.cancellation_penalty_percentage || 2}% Penalty: ₹{(order.total_amount * (config?.cancellation_penalty_percentage || 2) / 100).toFixed(0)})
                    </Button>
                  )}

                  {isPrimary && order.assigned_delivery_boy && (
                    <div className="pt-4 mt-3">
                      <div className="bg-green-50 border-2 border-green-500 p-3 rounded-lg">
                        <p className="text-xs font-semibold text-green-900 mb-1">ASSIGNED TO</p>
                        <p className="font-bold text-gray-900">{order.assigned_delivery_boy.name}</p>
                        <p className="text-sm text-gray-700">{order.assigned_delivery_boy.phone}</p>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
          ...
          <div className="space-y-4 py-4">
            <Alert className="bg-blue-50 border-blue-200">
              ...
              <AlertDescription className="text-blue-900 text-sm">
                Generate payment link from your payment gateway and paste it here. Link will be automatically sent to customer via their preferred channels.
              </AlertDescription>
            </Alert>
            {selectedOrder?.customer_payment_preference && selectedOrder.customer_payment_preference.length > 0 && (
              <div className="bg-green-50 border border-green-200 p-3 rounded-lg">
                <p className="text-sm font-medium text-green-900 mb-1">Send to customer via:</p>
                <div className="flex flex-wrap gap-2">
                  {selectedOrder.customer_payment_preference.map((channel, idx) => (
            ...
            <div className="text-sm text-gray-600">
              <p className="mb-2">Customer: <strong>{selectedOrder?.customer_name}</strong></p>
              <p>Amount: <strong>₹{selectedOrder?.total_amount}</strong></p>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowPaylinkDialog(false)}>
              Cancel
            </Button>
            <Button
