...
import { 
  X, User, Phone, Mail, Bike, Star, Package, 
  TrendingUp, Clock, Award, AlertCircle, CheckCircle
} from "lucide-react";
...
export default function RetailerDetails({ retailer, onClose, onUpdate }) {
  ...
  const handleStatusChange = async (newStatus) => {
    setUpdating(true);
    await Retailer.update(retailer.id, { status: newStatus });
    setUpdating(false);
    onUpdate();
  };
  ...
  const handleAvailabilityChange = async (newAvailability) => {
    setUpdating(true);
    await Retailer.update(retailer.id, { availability_status: newAvailability });
    setUpdating(false);
    onUpdate();
  };
  ...
  return (
    <Card className="border-none shadow-lg sticky top-4">
      <CardHeader className="border-b bg-white flex flex-row items-center justify-between">
        <CardTitle className="text-xl font-bold text-gray-900">Retailer Details</CardTitle>
      ...
      <CardContent className="p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          <div className="flex items-center justify-center gap-2">
            {retailer.rating && (
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-gray-900">{retailer.rating.toFixed(1)}</span>
              </div>
            )}
            <Badge className={`${
        ...
        <div>
          ...
          <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-semibold text-gray-900 capitalize">{retailer.vehicle_type}</span>
            </div>
            {retailer.vehicle_number && (
              <p className="text-sm text-gray-700 font-mono">{retailer.vehicle_number}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p className="text-2xl font-bold text-blue-700">{retailer.total_deliveries || 0}</p>
            </div>
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <p className="text-2xl font-bold text-green-700">{retailer.successful_deliveries || 0}</p>
            </div>
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-2xl font-bold text-amber-700">{retailer.acceptance_rate || 0}%</p>
            </div>
            <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
              ...
              <p className="text-2xl font-bold text-purple-700">{retailer.on_time_rate || 0}%</p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg">
              ...
              <p className="text-2xl font-bold text-green-700">₹{retailer.total_earnings || 0}</p>
            </div>
              ...
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xl font-bold text-amber-700">₹{retailer.pending_payout}</p>
                  </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <div>
              <label className="text-sm text-gray-600 mb-2 block">Priority Tier</label>
              <Select 
            ...
            <div>
              <label className="text-sm text-gray-600 mb-2 block">Availability</label>
              <Select 
